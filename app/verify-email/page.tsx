...
export default function VerifyEmailBridge() {
  ...
  useEffect(() => {
    const handleRedirect = () => {
      try {
        ...
        console.log('Email verification bridge page loaded with hash:', hash);

        if (!hash || hash.length <= 1) {
          setStatus('error');
          setErrorMessage('No verification tokens found in the link. Please request a new verification email.');
          return;
        }

        // Store hash in sessionStorage so we can retrieve it later if needed
        sessionStorage.setItem('verify_hash', hash);

        // Convert hash to query params for better iOS compatibility
        const hashParams = hash.startsWith('#') ? hash.substring(1) : hash;
        const appUrl = `strengthlens://auth/verify-email?${hashParams}`;
  ...
  const handleManualOpen = () => {
    // Try to get hash from current URL first, fallback to sessionStorage
    let hash = window.location.hash;
    if (!hash || hash.length <= 1) {
      hash = sessionStorage.getItem('verify_hash') || '';
    }
        ...
        {status === 'no-app' && (
          <>
            ...
            <p style={styles.description}>
              To complete your email verification, please open this link in the StrengthLens app.
            </p>
